...
const textColor = localStorage.getItem("chakra-ui-color-mode")
 ...
 if(response.data){
    ...
    try {
      if (editingProduct && editingProduct._id) {
        const res = await axios.put(
          `https://doeatsbackend.vercel.app/products/${editingProduct._id}`,
          editingProduct
        );
        fetchData();
        handleCloseModal();
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`https://doeatsbackend.vercel.app/products/delete/${id}`);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await axios.post("https://doeatsbackend.vercel.app/products/create", formData);
